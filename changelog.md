...
- Keybindings: `Shift+Alt+L` now opens compilelog.txt (was the linter), and `Ctrl+Shift+L` now runs the linter (was lowercase-transform). The `Ctrl+Alt+Shift+L` binding for the compile log and the `Ctrl+Shift+L` lowercase-transform binding are removed. (`Ctrl+Shift+U` still uppercases.)
- Debugger: on F5, optionally reveal the debug UI. Two independent toggles (both on by default): `qb64pe.debug.focusRunDebugViewOnStart` (Run and Debug sidebar view) and `qb64pe.debug.focusDebugConsoleOnStart` (Debug Console panel). Revealing also un-hides a pane that was collapsed.
